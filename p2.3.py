# -*- coding: utf-8 -*-
"""
Created on Fri Feb 19 23:53:25 2021
...
You'll notice that in Problem 2, your monthly payment had to be a multiple
 ...
 It should, but you may notice that your code runs more slowly, especially 
...
To recap the problem: we are searching for the smallest monthly payment
 ...
 but you can do better than that. If there was no interest, the debt can
 be paid off by monthly payments of one-twelfth of the original balance,
...
we paid off the entire balance at the end of the year.
 What we ultimately pay must be greater than what we would've paid in monthly
 ...
 would be one-twelfth of the balance, after having its interest compounded
...
Write a program that uses these bounds and bisection search
 ...
 and notice how fast it is (try the same large inputs in your solution to Problem 2 to compare!). Produce the same return value as you did in Problem 2.
"""
# balance is  starting balance
#air is annualInterestRate
# lmp is lowestmonthlyPayment
#ub unpaid balance
...
low = balance / 12.0
#Monthly payment upper bound = (Balance x (1 + Monthly interest rate)12) / 12.0
high =(balance * (1 +( air/12.0))**12) / 12.0
lmp=(low+high)/2.0
rb=balance
while rb==0:
   lmp= (low+high)/2.0# new midpoint
   rb= findlmp(balance,lmp)
   rba= findlmp(balance,low)
   rbb= findlmp(balance,high)
   if rb==0 or (high-low)/2.0 <tol : # solution found
        break
